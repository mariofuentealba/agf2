...
for($x = 1; $x < count($fff); $x++){
	...
	for($z = 1; $z < count($ff); $z++){
		...
		for($i = 0; $i < $arrEmpresas[$z - 1]; $i++){
			$empresa = new PHPExcel_Chart_DataSeriesValues('String', '' . str_replace(' ', '', $graf[$x]) . '!$' . $letras[$i] . '$' . (1 + $desp), NULL, 1);	//	'Budget'
		...
		$chart->setTopLeftPosition('J' . (2 + $desp));
		$chart->setBottomRightPosition('T' . (20 + $desp));
